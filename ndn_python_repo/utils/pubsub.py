        ...
        while n_retries > 0:
            try:
                ...
                data_name, meta_info, msg = await self.app.express_interest(
                    msg_int_name, int_param=int_param, validator=verify_ecdsa_signature)
                break
            except InterestNack as e:
                logging.debug(f'Nacked with reason: {e.reason}')
                await aio.sleep(1)
...
"""
    async def verify_ecdsa_signature(name: FormalName, sig: SignaturePtrs) -> bool:
        ...
        cert_name, meta_info, content, raw_packet = await app.express_interest(key_name,must_be_fresh=True, can_be_prefix=True,lifetime=6000,need_raw_packet=True)
