...
class SingleInsertClient(object):
    ...
    async def insert_single_msg(self, repo_prefix: Name, data_prefix: Name):
        face = self.face
        running = True
        ...
        if not isinstance(ret, Data):
            ...
            print('Decode succeeded: status code {}'.format(response.repo_command_response.status_code))
...
def main():
    ...
    event_loop = asyncio.get_event_loop()
    try:
        event_loop.run_until_complete(client.insert_single_msg(Name('testrepo'), Name('testdata')))
    finally:
        event_loop.close()
